...
public class DiningService extends BaseService {
    ...
    public void upsertDiningByForm(AdminDiningForm form) {
        ...
        if (rec == null) {
            ...
            .insertInto(
                ...
                , D_DINING.DAYS
                , D_DINING.ROOM_NO
				, D_DINING.CUSTOMER_NUM
                , D_DINING.BREAKFAST_JAPANESE
                , D_DINING.BREAKFAST_WESTERN
                , D_DINING.BREAKFAST_TIME
                , D_DINING.BREAKFAST_PLACE
                , D_DINING.DINNER
                , D_DINING.DINNER_TIME
                , D_DINING.DINNER_PLACE
				, D_DINING.LUNCH_NUM
                , D_DINING.LUNCH_TIME
                , D_DINING.MEMO
            ).values(
                ...
                , form.getRoomNo()
				, form.getCustomerNum()
                , form.getBreakfastJapanese()
                , form.getBreakfastWestern()
                , form.getBreakfastTime()
                , form.getBreakfastPlace()
                , form.getDinner()
                , form.getDinnerTime()
                , form.getDinnerPlace()
				, form.getLunchNum()
    ...
    public List<MDiningPlaceRecord> getDiningPlaceList(Integer kbn) {
		if (kbn == null) {
			...
			return jooq.selectFrom(M_DINING_PLACE).where(M_DINING_PLACE.KBN.eq(kbn)).orderBy(M_DINING_PLACE.SORT_ORDER).fetch();
		}
    }
    ...
    public boolean registCheck(AdminDiningForm form, int placeKbn) {
		...
		SelectConditionStep<DDiningRecord> cond = jooq
			...
			if (placeKbn == 1 ) {
				...
				// 夕方と朝の場所
				cond.and(D_DINING.BREAKFAST_PLACE.eq(form.getBreakfastPlace()).or(D_DINING.DINNER_PLACE.eq(form.getDinnerPlace())));
			}
    ...
    public DDiningMessageRecord getDiningMessage(String days, int diningPlaceId) {
		...
		// sqlのdateに変換
		java.sql.Date sqlDate = java.sql.Date.valueOf(days);
		return jooq.selectFrom(D_DINING_MESSAGE).where(D_DINING_MESSAGE.DAYS.eq(sqlDate).and(D_DINING_MESSAGE.DINING_PLACE_ID.eq(diningPlaceId))).fetchOne();		
    }
    ...
    public void upsertDiningMessageByForm(AdminDiningMessageForm form) {
        ...
        if (rec == null) {
            ...
            .insertInto(
                D_DINING_MESSAGE //
                , D_DINING_MESSAGE.DAYS
                , D_DINING_MESSAGE.DINING_PLACE_ID
				, D_DINING_MESSAGE.MESSAGE
            ).values(
                sqlDate
                , form.getDiningPlaceId()
				, form.getMessage()
            )
            .execute();
